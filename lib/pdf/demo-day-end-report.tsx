...
export async function renderDemoDayEndReport(data: { daily: DemoDailyReport[]; payments: DemoPaymentDetail[]; endingCash: number }) {
  ...
  return renderToBuffer(<Document title="VELOX Demo Gün Sonu Raporu" author="VELOX Demo"><Page size="A4" style={s.page}>
    ...
    {data.daily.map((row) => <Row key={row.date} values={[row.date, n(row.deposits), n(row.arbitrageGross), n(row.memberAccrual), n(row.referralExpense), n(row.networkIncome), n(row.totalDistribution), n(row.automaticPayments), n(row.profitLoss), n(row.closingCash)]} />)}
    ...
    {data.payments.slice(-30).reverse().map((item, index) => <Row key={`${item.reference}-${index}`} values={[item.date, item.name, item.veloxId, item.type === 'demo_auto_withdrawal' ? 'Demo oto. çekim' : item.type === 'demo_referral_commission' ? 'Doğrudan ref. %6' : 'Network geliri', n(item.amount), item.reference]} />)}
