...
export function MatrixNetworkDashboard(props: {
  ...
  const levelMembers = useMemo(() => safeArray<NetworkListRow>(props.memberList).filter((item) => item.level === level), [level, props.memberList])
...
function Overview({ summary, referralCode, legs, depthRows, marketScenarios, cashbackQualification, demoFinance }: Pick<Parameters<typeof MatrixNetworkDashboard>[0], 'summary' | 'referralCode' | 'legs' | 'depthRows' | 'marketScenarios' | 'cashbackQualification' | 'demoFinance'>) {
  const openLevels = safeArray<DepthRow>(depthRows).filter((item) => item.unlocked).length
  const activeScenario = safeArray<DemoMarketScenario>(marketScenarios).find((item) => item.active) ?? marketScenarios[0]
  const networkIncomePreview = safeArray<DepthRow>(depthRows)
  ...
  return <>
    ...
    <Panel className="mt-4"><PanelHeader title="Gün içi demo piyasa senaryosu" right={<span className="text-xs text-muted-foreground">Dağıtım %1,40–%2,20 · günlük ortalama %1,70 · brüt demo %2,60</span>} /><div className="grid gap-2 p-4 sm:grid-cols-2 lg:grid-cols-4">{safeArray<DemoMarketScenario>(marketScenarios).map((scenario) => <div key={scenario.slot} className={scenario.active ? 'rounded-md border border-cyan/40 bg-cyan/10 px-3 py-2.5' : 'rounded-md border border-border bg-elevated px-3 py-2.5'}><p className="text-xs text-muted-foreground">{scenario.label}{scenario.active ? ' · aktif' : ''}</p><p className="mt-1 font-mono text-lg font-semibold text-foreground">%{scenario.rate.toFixed(2)}</p></div>)}</div><p className="px-4 pb-4 text-xs text-muted-foreground">Bu değer yalnızca günlük demo senaryosudur; hiçbir ödeme, hak ediş veya çekim kaydı üretmez.</p></Panel>
