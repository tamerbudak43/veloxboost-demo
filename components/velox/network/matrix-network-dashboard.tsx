...
import { DEMO_DAILY_ARBITRAGE_RATE } from '@/lib/network/demo-commission-plan'
...
function Overview({ summary, referralCode, legs, depthRows }: Pick<Parameters<typeof MatrixNetworkDashboard>[0], 'summary' | 'referralCode' | 'legs' | 'depthRows'>) {
  ...
  const networkIncomePreview = safeArray(depthRows)
    ...
    .reduce((total, item) => total + safeNumber(item.investment) * (DEMO_DAILY_ARBITRAGE_RATE / 100) * (safeNumber(item.commissionRate) / 100), 0)
  return <>
