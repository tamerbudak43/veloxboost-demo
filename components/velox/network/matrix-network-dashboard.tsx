...
function Overview({ summary, referralCode, legs, depthRows, marketScenarios, simulation }: Pick<Parameters<typeof MatrixNetworkDashboard>[0], 'summary' | 'referralCode' | 'legs' | 'depthRows' | 'marketScenarios' | 'simulation'>) {
  ...
  return <>
    ...
    <div className="mt-4 grid gap-4 lg:grid-cols-2"><Panel><PanelHeader title="Tüm takım cirosu" right={<GitBranch className="size-4 text-cyan" />} /><div className="p-5"><p className="font-mono text-3xl font-semibold tabular-nums text-foreground">{formatUSDT(summary.teamVolume, 2)}</p><p className="mt-1 text-sm text-muted-foreground">Tüm seviyelerdeki kayıtlı ekip cirosu</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><Metric label="Direkt ciro" value={formatUSDT(summary.directVolume, 2)} /><Metric label="Güçlü bacak" value={formatUSDT(summary.strongLegVolume, 2)} /><Metric label="Diğer bacaklar" value={formatUSDT(summary.otherLegVolume, 2)} /><Metric label="Aktif ortak" value={summary.activePartners} /></div></div></Panel><Panel><PanelHeader title="Network geliri" right={<StatusPill tone="warning">Demo tahakkuk</StatusPill>} /><div className="p-5"><p className="font-mono text-3xl font-semibold tabular-nums text-light-cyan">{formatUSDT(networkIncomePreview, 2)}</p><p className="mt-1 text-sm text-muted-foreground">Doğrulanmış demo yatırım × aktif senaryo %{activeScenario?.rate ?? 0} × derinlik oranı</p><div className="mt-5 grid gap-3 sm:grid-cols-2"><Metric label="Aktif zaman dilimi" value={activeScenario?.label ?? '—'} /><Metric label="Mevcut kariyer" value={summary.currentCareer} /></div><p className="mt-4 text-xs leading-5 text-amber-200">Bu tutar ödeme, bakiye veya çekim hakkı değildir; yalnızca ağ geliri görünümünün demo hesaplamasıdır.</p></div></Panel></div>
    <Panel className="mt-4"><PanelHeader title="Gün içi demo piyasa senaryosu" right={<span className="text-xs text-muted-foreground">8 zaman dilimi · %1,4–%2,1</span>} /><div className="grid gap-2 p-4 sm:grid-cols-2 lg:grid-cols-4">{safeArray(marketScenarios).map((scenario) => <div key={scenario.slot} className={scenario.active ? 'rounded-md border border-cyan/40 bg-cyan/10 px-3 py-2.5' : 'rounded-md border border-border bg-elevated px-3 py-2.5'}><p className="text-xs text-muted-foreground">{scenario.label}{scenario.active ? ' · aktif' : ''}</p><p className="mt-1 font-mono text-lg font-semibold text-foreground">%{scenario.rate.toFixed(2)}</p></div>)}</div><p className="px-4 pb-4 text-xs text-muted-foreground">Saatlik değerler günlük demo senaryosu için sabitlenir; hiçbir ödeme, hak ediş veya çekim kaydı üretmez.</p></Panel>
    {simulation && <GrowthSimulation simulation={simulation} />}
    {simulation && <DemoAutoSettlement summary={summary} depthRows={depthRows} simulation={simulation} networkIncome={networkIncomePreview} />}
