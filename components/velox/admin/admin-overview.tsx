    ...
    <Panel className="border-cyan/30 p-0">
      ...
      <div className="grid gap-4 border-t border-border/60 p-4 lg:grid-cols-[1.2fr_.8fr]">
        <div>
          ...
          {daily.length === 0 ? <div className="py-10 text-center text-sm text-muted-foreground">Demo henüz çalıştırılmadı.</div> : <div className="flex h-52 items-end gap-3 rounded-lg border border-border/60 bg-surface/40 p-3">{daily.map((row) => { const height = (value: number) => Math.max(8, Math.sqrt(value / chartMax) * 100); return <div key={row.date} className="flex min-w-0 flex-1 flex-col items-center gap-2 rounded-md px-1 py-1 hover:bg-background/40" title={`${row.date}: giriş ${formatUSDT(row.deposits, 2)} · yatırım dağıtımı ${formatUSDT(row.memberAccrual, 2)} · referral ${formatUSDT(row.referralExpense, 2)} · network ${formatUSDT(row.networkIncome, 2)}`}><div className="flex h-40 w-full items-end justify-center gap-1"><div className="w-3 rounded-t bg-cyan shadow-[0_0_10px_rgba(34,211,238,.5)]" style={{ height: `${height(row.deposits)}%` }} /><div className="w-3 rounded-t bg-electric" style={{ height: `${height(row.memberAccrual)}%` }} /><div className="w-3 rounded-t bg-amber-400" style={{ height: `${height(row.referralExpense)}%` }} /><div className="w-3 rounded-t bg-fuchsia-400" style={{ height: `${height(row.networkIncome)}%` }} /></div><span className="font-mono text-[10px] text-muted-foreground">{row.date.slice(5)}</span></div> })}</div>}
          <div className="mt-3 flex flex-wrap justify-center gap-x-5 gap-y-2 text-[11px] text-muted-foreground"><span><i className="mr-1.5 inline-block size-2.5 rounded-sm bg-cyan" />Giriş</span><span><i className="mr-1.5 inline-block size-2.5 rounded-sm bg-electric" />Yatırım kârı</span><span><i className="mr-1.5 inline-block size-2.5 rounded-sm bg-amber-400" />Referral %6</span><span><i className="mr-1.5 inline-block size-2.5 rounded-sm bg-fuchsia-400" />Network</span></div>
        </div>
