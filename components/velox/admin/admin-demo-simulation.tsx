...
const ENTRY_LABEL: Record<string, string> = {
  ...
  demo_referral_commission: 'Doğrudan referral %6',
  demo_network_commission: 'Network geliri',
...
export function AdminDemoSimulation({ members, ledger }: { members: DemoMember[]; ledger: Ledger[] }) {
  ...
  const referralCommissions = entries.filter((item) => item.entryType === 'demo_referral_commission').reduce((sum, item) => sum + safeNumber(item.amount), 0)
  ...
  function sponsorName(sponsorId: string | null) {
    return sponsorId ? rows.find((item) => item.userId === sponsorId)?.name ?? 'DEMO ağ üyesi' : 'Ana demo hesap'
  }

  function commissionSource(reference: string) {
    const sourceId = reference.match(/^DEMO-(.+)-D\d+-(?:REF6|L\d+-NET)$/)?.[1]
    return sourceId ? rows.find((item) => item.veloxId === sourceId)?.name ?? sourceId : null
  }
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5"><StatTile label="Demo üye" value={formatNumber(rows.length, 0)} /><StatTile label="Demo yatırım" value={formatUSDT(investments, 2)} accent /><StatTile label="Üye tahakkuku" value={formatUSDT(accruals, 2)} /><StatTile label="Referral %6" value={formatUSDT(referralCommissions, 2)} accent /><StatTile label="Demo otomatik çekim" value={formatUSDT(withdrawals, 2)} /></div>
    <Panel className="p-0"><PanelHeader title="Demo üye hesapları" right={<StatusPill tone="warning">Sentetik kayıt</StatusPill>} />{rows.length === 0 ? <div className="px-5 py-10 text-center text-sm text-muted-foreground">Henüz demo verisi kurulmadı.</div> : <div className="max-h-[420px] divide-y divide-border overflow-y-auto">{rows.map((item) => <div key={item.userId} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3"><div><p className="font-semibold text-foreground">{item.name} <span className="font-mono text-xs text-muted-foreground">{item.veloxId}</span></p><p className="mt-1 text-xs text-muted-foreground">{item.career} · Sponsor: {sponsorName(item.sponsorId)} · {formatDateTime(item.createdAt)}</p></div><p className="font-mono text-sm font-semibold text-foreground">{formatUSDT(item.personalInvestment, 2)}</p></div>)}</div>}</Panel>
    <Panel className="p-0"><PanelHeader title="Demo işlem defteri" right={<span className="text-xs text-muted-foreground">Son {entries.length} kayıt</span>} />{entries.length === 0 ? <div className="px-5 py-10 text-center text-sm text-muted-foreground">Demo işlem kaydı yok.</div> : <div className="max-h-[520px] divide-y divide-border overflow-y-auto">{entries.map((item) => { const source = item.entryType === 'demo_referral_commission' || item.entryType === 'demo_network_commission' ? commissionSource(item.reference) : null; const sourceLabel = item.entryType === 'demo_referral_commission' ? `${source} yatırımı × %6` : item.entryType === 'demo_network_commission' ? `${source} günlük kârı üzerinden` : null; return <div key={item.id} className="flex flex-wrap items-center justify-between gap-3 px-5 py-3"><div><p className="font-medium text-foreground">{ENTRY_LABEL[item.entryType] ?? item.entryType} <span className="ml-2 font-mono text-xs text-muted-foreground">{item.reference}</span></p><p className="mt-1 text-xs text-muted-foreground">{item.userName} · {item.veloxId} · {sourceLabel ? `${sourceLabel} · ` : ''}{formatDateTime(item.occurredAt)}</p></div><div className="text-right"><p className={item.amount < 0 ? 'font-mono text-sm font-semibold text-rose-300' : 'font-mono text-sm font-semibold text-emerald-300'}>{item.amount < 0 ? '−' : '+'}{formatUSDT(Math.abs(item.amount), 2)}</p><p className="mt-1 text-[11px] text-muted-foreground">{item.status}</p></div></div>})}</div>}</Panel>
  </div>
}
